...
| --- | --- | --- |
| 0. Discovery and foundations | complete | Local Git initialized; governance, architecture, product, threat-model, license, locks, immutable GitHub Action pins, and release policy are present. Repository policy checks are part of the 44-test Python gate and now require the manifest inventory to exactly match the payload. |
| 1. CLI foundation | complete | `python infrastructure/scripts/verify_release.py` passes Ruff, formatting, strict mypy, 44 Python tests, exact dependency audits, clean wheel installation, `--version`, JSON `doctor`, website/SaaS creation, and `pip check` on Windows. Doctor command timeouts are stable required failures or optional warnings. Click 8.3.3 is locked to remediate `PYSEC-2026-2132`; installation beside Bench is explicitly isolated. |
...
| 3. Frappe environments | in progress | Development/production Compose configurations validate; all images and upstream source commits are pinned. Both profiles in [container run 30324235882](https://github.com/5bv57zcm44-max/noxus-core/actions/runs/30324235882) built the pinned Frappe/ERPNext sources, installed all nine apps and 53 DocTypes, started healthy services, passed the public versioned health API, verified installed apps, ran the Core Frappe tests, completed two-real-site isolation, and created backups. Frappe v16 then decoded its gzip database dump as UTF-8 during restore preflight. NOXUS now privately stages gzip dumps as uncompressed SQL, preserves the mounted archive, removes the staging file, and keeps subsequent restore errors intact; four local regression tests cover the boundary. A replacement live restore run remains outstanding. |
...
| 7. Hardening and documentation | in progress | On 2026-07-28, local qualification passed Ruff, formatting, strict mypy, 44 Python tests, ESLint, TypeScript, Vitest, Playwright/WCAG, pip-audit, and npm audit with no known vulnerabilities. [CI run 30324250664](https://github.com/5bv57zcm44-max/noxus-core/actions/runs/30324250664) passed on Linux/Windows with Python 3.11/3.14 and the full frontend gate; [security run 30324250666](https://github.com/5bv57zcm44-max/noxus-core/actions/runs/30324250666) passed Gitleaks, pip-audit, and npm audit. Container Trivy, release SBOM, and provenance remain outstanding. |
