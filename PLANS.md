...
| --- | --- | --- |
| 0. Discovery and foundations | complete | Local Git initialized; governance, architecture, product, threat-model, license, locks, immutable GitHub Action pins, and release policy are present. Repository policy checks are part of the 37-test Python gate and now require the manifest inventory to exactly match the payload. |
| 1. CLI foundation | complete | `python infrastructure/scripts/verify_release.py` passes Ruff, formatting, strict mypy, 37 Python tests, exact dependency audits, clean wheel installation, `--version`, JSON `doctor`, website/SaaS creation, and `pip check` on Windows. Doctor command timeouts are stable required failures or optional warnings. Click 8.3.3 is locked to remediate `PYSEC-2026-2132`; installation beside Bench is explicitly isolated. |
...
| 3. Frappe environments | in progress | Development/production Compose configurations validate; all images and upstream source commits are pinned. Both profiles in [container run 30321996625](https://github.com/5bv57zcm44-max/noxus-core/actions/runs/30321996625) built and installed all nine NOXUS apps, proving dependency-ordered creation of all 53 bundled DocTypes. Runtime startup then exposed a site-unaware backend health probe and YAML-split worker queue arguments. Both Compose contracts and their repository regression test are corrected; a replacement live run remains outstanding. |
...
| 7. Hardening and documentation | in progress | On 2026-07-28, local qualification passed Ruff, formatting, strict mypy, 37 Python tests, ESLint, TypeScript, Vitest, Playwright/WCAG, pip-audit, and npm audit with no known vulnerabilities. [CI run 30321996550](https://github.com/5bv57zcm44-max/noxus-core/actions/runs/30321996550) passed on Linux/Windows with Python 3.11/3.14 and the full frontend gate; [security run 30322009453](https://github.com/5bv57zcm44-max/noxus-core/actions/runs/30322009453) passed Gitleaks, pip-audit, and npm audit. Container Trivy, release SBOM, and provenance remain outstanding. |
