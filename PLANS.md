...
| --- | --- | --- |
| 0. Discovery and foundations | complete | Local Git initialized; governance, architecture, product, threat-model, license, locks, immutable GitHub Action pins, and release policy are present. Repository policy checks are part of the 35-test Python gate and now require the manifest inventory to exactly match the payload. |
| 1. CLI foundation | complete | `python infrastructure/scripts/verify_release.py` passes Ruff, formatting, strict mypy, 35 Python tests, exact dependency audits, clean wheel installation, `--version`, JSON `doctor`, website/SaaS creation, and `pip check` on Windows. Doctor command timeouts are stable required failures or optional warnings. Click 8.3.3 is locked to remediate `PYSEC-2026-2132`; installation beside Bench is explicitly isolated. |
...
| 3. Frappe environments | in progress | Development/production Compose configurations validate; all images and upstream source commits are pinned. [Container run 30320803554](https://github.com/5bv57zcm44-max/noxus-core/actions/runs/30320803554) installed Core, AI, CRM, and Education, then Frappe v16 rejected Inventory's optional ERPNext Dynamic Link because its type selector was not a `Link` to `DocType`. The adapter and a repository-wide Dynamic Link contract test are corrected; a replacement live run remains outstanding. |
