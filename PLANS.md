...
| --- | --- | --- |
| 0. Discovery and foundations | complete | Local Git initialized; governance, architecture, product, threat-model, license, locks, immutable GitHub Action pins, and release policy are present. Repository policy checks are part of the 38-test Python gate and now require the manifest inventory to exactly match the payload. |
| 1. CLI foundation | complete | `python infrastructure/scripts/verify_release.py` passes Ruff, formatting, strict mypy, 38 Python tests, exact dependency audits, clean wheel installation, `--version`, JSON `doctor`, website/SaaS creation, and `pip check` on Windows. Doctor command timeouts are stable required failures or optional warnings. Click 8.3.3 is locked to remediate `PYSEC-2026-2132`; installation beside Bench is explicitly isolated. |
...
| 3. Frappe environments | in progress | Development/production Compose configurations validate; all images and upstream source commits are pinned. Both profiles in [container run 30322765890](https://github.com/5bv57zcm44-max/noxus-core/actions/runs/30322765890) built, installed all nine NOXUS apps, started healthy backend/workers, and reached the versioned API through the proxy. Frappe then correctly denied the nominally public health method because its decorator lacked explicit guest access. The minimal guest response is now explicitly allowed and protected by a contract test; a replacement live run remains outstanding. |
...
| 7. Hardening and documentation | in progress | On 2026-07-28, local qualification passed Ruff, formatting, strict mypy, 38 Python tests, ESLint, TypeScript, Vitest, Playwright/WCAG, pip-audit, and npm audit with no known vulnerabilities. [CI run 30322765898](https://github.com/5bv57zcm44-max/noxus-core/actions/runs/30322765898) passed on Linux/Windows with Python 3.11/3.14 and the full frontend gate; [security run 30322776732](https://github.com/5bv57zcm44-max/noxus-core/actions/runs/30322776732) passed Gitleaks, pip-audit, and npm audit. Container Trivy, release SBOM, and provenance remain outstanding. |
