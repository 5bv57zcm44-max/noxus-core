...
| 2. Website generator | in progress | Source and clean-wheel generated SQLite projects migrate and their seven emitted tests pass. Development and production Compose profiles and a PostgreSQL acceptance script are present. The first Linux container run exposed the PostgreSQL 18 volume-layout change; the template now mounts `/var/lib/postgresql` and the acceptance harness preserves logs and cleans partial starts. A green rerun remains outstanding. |
| 3. Frappe environments | in progress | Development/production Compose configurations validate; all images and upstream source commits are pinned. The first Linux image build exposed Bench's Yarn prerequisite; the backend image now installs pinned Yarn 1.22.22. Live Frappe-only and ERPNext acceptance reruns remain outstanding. |
