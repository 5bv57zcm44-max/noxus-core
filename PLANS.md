...
| --- | --- | --- |
| 0. Discovery and foundations | complete | Local Git initialized; governance, architecture, product, threat-model, license, locks, immutable GitHub Action pins, and release policy are present. Repository policy checks are part of the 46-test Python gate and now require the manifest inventory to exactly match the payload. |
| 1. CLI foundation | complete | `python infrastructure/scripts/verify_release.py` passes Ruff, formatting, strict mypy, 46 Python tests, exact dependency audits, clean wheel installation, `--version`, JSON `doctor`, website/SaaS creation, and `pip check` on Windows. Doctor command timeouts are stable required failures or optional warnings. Click 8.3.3 is locked to remediate `PYSEC-2026-2132`; installation beside Bench is explicitly isolated. |
...
| 7. Hardening and documentation | in progress | On 2026-07-28, local qualification passed Ruff, formatting, strict mypy, 46 Python tests, ESLint, TypeScript, Vitest, Playwright/WCAG, pip-audit, and npm audit with no known vulnerabilities. [CI run 30325260338](https://github.com/5bv57zcm44-max/noxus-core/actions/runs/30325260338) passed on Linux/Windows with Python 3.11/3.14 and the full frontend gate; [security run 30325260291](https://github.com/5bv57zcm44-max/noxus-core/actions/runs/30325260291) passed Gitleaks, pip-audit, npm audit, repository Trivy, and SARIF upload. The first container scan exposed Frappe's build-time Node tree and caches, fixable Python packages behind upstream conservative constraints, and Debian Bookworm advisories without vendor fixes. The replacement keeps digest-pinned Bookworm for Frappe's required `wkhtmltopdf`, uses exact audited Socket.IO-only runtime dependencies, applies patched Python overrides exercised by the complete Frappe suites, and removes build/package-manager material. Trivy continues to fail on fixable HIGH/CRITICAL findings while unfixed vendor findings are tracked by the SBOM and weekly scans. Replacement image Trivy, release SBOM, and provenance remain outstanding. |
