...
| --- | --- | --- |
| 0. Discovery and foundations | complete | Local Git initialized; governance, architecture, product, threat-model, license, locks, immutable GitHub Action pins, and release policy are present. Repository policy checks are part of the 45-test Python gate and now require the manifest inventory to exactly match the payload. |
| 1. CLI foundation | complete | `python infrastructure/scripts/verify_release.py` passes Ruff, formatting, strict mypy, 45 Python tests, exact dependency audits, clean wheel installation, `--version`, JSON `doctor`, website/SaaS creation, and `pip check` on Windows. Doctor command timeouts are stable required failures or optional warnings. Click 8.3.3 is locked to remediate `PYSEC-2026-2132`; installation beside Bench is explicitly isolated. |
...
| 3. Frappe environments | in progress | Development/production Compose configurations validate; all images and upstream source commits are pinned. Both profiles in [container run 30325249507](https://github.com/5bv57zcm44-max/noxus-core/actions/runs/30325249507) built the pinned Frappe/ERPNext sources, installed all nine apps and 53 DocTypes, started healthy services, passed the public versioned health API, verified installed apps, ran the Core Frappe tests, completed two-real-site isolation, created backups, and successfully passed gzip input into database restore. The post-restore migration then exposed a root-owned site-lock directory. The restore entrypoint now reads the protected secret and mounted archive as root, creates a private `frappe`-owned copy, and drops privileges before Frappe initialization so all site locks retain runtime ownership. A replacement live restore/restart run remains outstanding. |
...
| 7. Hardening and documentation | in progress | On 2026-07-28, local qualification passed Ruff, formatting, strict mypy, 45 Python tests, ESLint, TypeScript, Vitest, Playwright/WCAG, pip-audit, and npm audit with no known vulnerabilities. [CI run 30325260338](https://github.com/5bv57zcm44-max/noxus-core/actions/runs/30325260338) passed on Linux/Windows with Python 3.11/3.14 and the full frontend gate; [security run 30325260291](https://github.com/5bv57zcm44-max/noxus-core/actions/runs/30325260291) passed Gitleaks, pip-audit, npm audit, repository Trivy, and SARIF upload. Container-image Trivy, release SBOM, and provenance remain outstanding. |
