...
| 3. Frappe environments | in progress | Development/production Compose configurations validate; all images and upstream source commits are pinned. [Container run 30318938864](https://github.com/5bv57zcm44-max/noxus-core/actions/runs/30318938864) validated both image builds and protected-secret privilege drop, then showed the programmatic creator searching `./apps.txt` instead of Bench's `sites/apps.txt`. Site initialization now passes the explicit Bench sites directory; a replacement live run remains outstanding. |
