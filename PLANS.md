...
| --- | --- | --- |
| 0. Discovery and foundations | complete | Local Git initialized; governance, architecture, product, threat-model, license, locks, immutable GitHub Action pins, and release policy are present. Repository policy checks are part of the 31-test Python gate and now require the manifest inventory to exactly match the payload. |
| 1. CLI foundation | complete | `python infrastructure/scripts/verify_release.py` passes Ruff, formatting, strict mypy, 31 Python tests, exact dependency audits, clean wheel installation, `--version`, JSON `doctor`, website/SaaS creation, and `pip check` on Windows. Click 8.3.3 is locked to remediate `PYSEC-2026-2132`; installation beside Bench is explicitly isolated. |
...
| 3. Frappe environments | in progress | Development/production Compose configurations validate; all images and upstream source commits are pinned. [Container run 30319465157](https://github.com/5bv57zcm44-max/noxus-core/actions/runs/30319465157) validated secret loading and app discovery, then showed Frappe's relative logger resolving outside the Bench volume. The bootstrap now changes into the explicit Bench `sites` directory, matching Frappe CLI behavior for both app discovery and `../logs`; a replacement live run remains outstanding. |
...
| 7. Hardening and documentation | in progress | On 2026-07-28, `python infrastructure/scripts/verify_release.py` passed Ruff, formatting, strict mypy, 31 Python tests, ESLint, TypeScript, Vitest, Playwright/WCAG, pip-audit, and npm audit with no known vulnerabilities. [CI run 30318938877](https://github.com/5bv57zcm44-max/noxus-core/actions/runs/30318938877) passed on Linux/Windows with Python 3.11/3.14 and the full frontend gate; [security run 30318951418](https://github.com/5bv57zcm44-max/noxus-core/actions/runs/30318951418) passed Gitleaks, pip-audit, and npm audit. Container Trivy, release SBOM, and provenance remain outstanding. |
