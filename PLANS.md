...
| --- | --- | --- |
| 0. Discovery and foundations | complete | Local Git initialized; governance, architecture, product, threat-model, license, locks, immutable GitHub Action pins, and release policy are present. Repository policy checks are part of the 27-test Python gate and now require the manifest inventory to exactly match the payload. |
| 1. CLI foundation | complete | `python infrastructure/scripts/verify_release.py` passes Ruff, formatting, strict mypy, 27 Python tests, exact dependency audits, clean wheel installation, `--version`, JSON `doctor`, website/SaaS creation, and `pip check` on Windows. Click 8.3.3 is locked to remediate `PYSEC-2026-2132`; installation beside Bench is explicitly isolated. |
| 2. Website generator | in progress | Source and clean-wheel generated SQLite projects migrate and their seven emitted tests pass. Development and production Compose profiles and a PostgreSQL acceptance script are present. Linux container runs exposed the PostgreSQL 18 volume-layout change and protected Compose secret ownership; the template now mounts `/var/lib/postgresql`, reads secrets before dropping to `django`, and preserves logs plus cleanup for partial starts. A green rerun remains outstanding. |
| 3. Frappe environments | in progress | Development/production Compose configurations validate; all images and upstream source commits are pinned. Linux image builds exposed Bench's Yarn and `crontab` prerequisites; the backend image now installs pinned Yarn 1.22.22 and Debian's cron package. Live Frappe-only and ERPNext acceptance reruns remain outstanding. |
...
| 7. Hardening and documentation | in progress | On 2026-07-28, `python infrastructure/scripts/verify_release.py` passed Ruff, formatting, strict mypy, 27 Python tests, ESLint, TypeScript, Vitest, Playwright/WCAG, pip-audit, and npm audit with no known vulnerabilities after clean-CI dependency corrections. Trivy is pinned to v0.36.0 by commit, its SARIF inputs are valid YAML, and missing reports no longer mask the primary failure. CI-hosted Trivy, Gitleaks, SBOM, and provenance reruns remain outstanding. |
...
- The authorized fallback repository under `5bv57zcm44-max` supplied standard GitHub-hosted runners.
  Initial runs exposed four reproducible repository issues rather than account failures: clean CI
  lacked the generated Django test dependencies, PostgreSQL 18 rejected its old data mount, the
  Frappe image lacked Yarn, and the Trivy workflow had an invalid unquoted severity value plus an
  obsolete action revision. Those causes are corrected and locally qualified; replacement GitHub
  runs must be green before publication.
